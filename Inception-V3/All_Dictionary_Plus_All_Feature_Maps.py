...
slice_names = np.append(slice_names, np.array(models.feature_extraction.get_graph_node_names(inception)[0]))
...
for i in range(len(images)):
    ...
    inception.eval()
    inception(images_transformed_un_squeezed[i])
    j = 0
    for j in range(slice_names.size):
        print([slice_names[j]])
        slice_output = create_feature_extractor(inception, [slice_names[j]])
        with torch.no_grad():
    ...
    pickle.dump(all_images_all_layers[image_names_notif[i]],
                open("C:\\Users\\Asus\\Desktop\\Project\\Inception-V3\\CNN Layer outputs in tensor as pickle\\" +
                     image_names_notif[
                         i] + '_tensor.p', "wb"))
...
for Q in range(len(images)):
    ...
    image_dictionary = pd.read_pickle(
        r'C:\\Users\\Asus\\Desktop\\Project\\Inception-V3\\CNN Layer outputs in tensor as pickle\\' + image_names_notif[
            Q] + '_tensor.p')
    ...
    plt.savefig(
        'C:\\Users\\Asus\\Desktop\\Project\\Inception-V3\\Images feature maps\\' + image_names_notif[Q] + '.png',
        bbox_inches='tight')
    plt.close()
