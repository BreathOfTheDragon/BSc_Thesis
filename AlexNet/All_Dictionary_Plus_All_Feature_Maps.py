...
for i in range(len(images)):
    ...
    alexnet.eval()
    alexnet(images_transformed_un_squeezed[i])
    j = 0
    for j in range(slice_names.size):
        slice_output = create_feature_extractor(alexnet, return_nodes=[slice_names[j]])
        with torch.no_grad():
    ...
    pickle.dump(all_images_all_layers[image_names_notif[i]],
                open("C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\CNN Layer outputs in tensor as pickle\\" +
                     image_names_notif[
                         i] + '_tensor.p', "wb"))
...
for Q in range(len(images)):
    ...
    image_dictionary = pd.read_pickle(
        r'C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\CNN Layer outputs in tensor as pickle\\' + image_names_notif[
            Q] + '_tensor.p')
    ...
    for i in range(len(processed)):
        ...
        a.axis("off")
        a.set_title(conv_slices[i].split('(')[0], fontsize=25)
    plt.savefig('C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\Images feature maps\\' + image_names_notif[Q] + '.png',
                bbox_inches='tight')
    plt.close()
