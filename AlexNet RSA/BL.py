...
# This code imports the data of brain neurons (352 neurons) for type BL categories (human_face , inanimate & ...)
# also imports data of all neurons (each layer has many neurons) of all layers of a NN for the same type BL categories
...
# in the end , plots the correlation of brain for type BL categories and each of the NN layers for type BL categories
...
# in the end we will have #(NN_layers) numbers , for example 22 for AlexNet (excluding X)
# for example :
...
type_of_image = ['Human_Face_LF', 'Animal_Face_LF', 'Human_Body_LF', 'Animal_Body_LF', 'Natural_LF', 'Man_Made_LF',
                 'Face_LF', 'Body_LF', 'Animate_LF', 'Inanimate_LF', 'All_LF']
...
for mk in range(len(type_of_image)):
    ...
    for i in range(len(slice_names)):
        sliceF_x_image = pd.read_pickle(
            r'C:\Users\Asus\Desktop\Project\AlexNet\CNN Slices Flattened by Image as pickle\\' + str(i).zfill(3) + '_' +
            slice_names[
                i] + '_for_all_images' + '.p')
    ...
    scipy.io.savemat(
        'C:/Users/Asus/Desktop/Project/AlexNet/RDM Comparisons B mats/' + str(k + 1) + '_RDM_comparison_' +
        type_of_image[
            k] + '.mat',
...
# <editor-fold desc="plotting type BL correlations between brain and each NN layer , for each type BL category , each in 1 plot">
comparison_mats_directory = "C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\RDM Comparisons B mats\\"
comparison_mats = []
...
for l in range(len(comparison_mats)):
    ...
    plt.title("Correlation between brain region and AlexNet Slices", fontsize=25)
    plt.draw()
    plt.savefig(
        'C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\RDM Comparisons B Plots\\' + str(l + 1) + '_' + type_of_image[
            l] + '.png',
        dpi=100)
    plt.close()
# </editor-fold>

# <editor-fold desc="plotting type BL correlations between brain and each NN layer , for each type BL category , all in 1 plot">
plt.figure(figsize=(20, 20))
plt.yticks(fontsize=20)
plt.ylim(-1.0, 1.0)
plt.xticks(fontsize=10, rotation=90)
...
plt.draw()
plt.savefig(
    'C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\RDM Comparisons B Plots\\Combined_LF.png',
    dpi=100)
plt.close()
# </editor-fold>

# <editor-fold desc="plotting which layer has the highest correlation for each type BL category , aka best layer">
max_comparisons = []
max_indices = []
...
plt.draw()
plt.savefig('C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\RDM Comparisons MAX Plots\\MAX_BL_Types.png',
            dpi=100)
plt.close()
# </editor-fold>

# <editor-fold desc="plotting what is the correlation value of the best layer for each type BL category , aka max correlation">
plt.figure(figsize=(25, 25))
...
plt.draw()
plt.savefig('C:\\Users\\Asus\\Desktop\\Project\\AlexNet\\RDM Comparisons Best Layer Plots\\Best_Layer_BL_Types.png',
            dpi=100)
plt.close()
# </editor-fold>
