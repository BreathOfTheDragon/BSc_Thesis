...
#   |  this code uses 6 face images , all faces are human , and 0 animal face  |
#   ****************************************************************************
...
print(ax1)
accuracies = []
train_input_BH = np.transpose(neuron_output[:, np.r_[74:80, 83:89]])
train_input_BI = np.transpose(neuron_output[:, np.r_[101:107, 110:116]])
train_input_BL = np.transpose(neuron_output[:, np.r_[128:134, 137:143]])

test_input_BH = np.transpose(neuron_output[:, np.r_[74:80, 83:89]])
test_input_BI = np.transpose(neuron_output[:, np.r_[101:107, 110:116]])
test_input_BL = np.transpose(neuron_output[:, np.r_[128:134, 137:143]])
...
plt.draw()
plt.savefig(
    'C:\\Users\\Asus\\Desktop\\Project\\Comparing Models Last Layer\\BX_BX\\bodyface double human face\\'
