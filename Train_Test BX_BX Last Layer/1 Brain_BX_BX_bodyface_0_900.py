...
for kkk in range(len(neuron_output)):
    ...
    train_input_BH = np.transpose(
        neuron_output[kkk]["neurons_stacked_averaged_for_" + str(0 + 5 * kkk) + "_to_" + str(49 + 5 * kkk)][:, 77:89])
    train_input_BI = np.transpose(
        neuron_output[kkk]["neurons_stacked_averaged_for_" + str(0 + 5 * kkk) + "_to_" + str(49 + 5 * kkk)][:, 104:116])
    train_input_BL = np.transpose(
        neuron_output[kkk]["neurons_stacked_averaged_for_" + str(0 + 5 * kkk) + "_to_" + str(49 + 5 * kkk)][:, 131:143])
...
for lll in range(len(labels)):
    ...
    plt.draw()
    plt.savefig(
        'C:\\Users\\Asus\\Desktop\\Project\\Comparing Models Last Layer\\BX_BX\\bodyface\\Brain BX_BX_Accuracy 0_900\\'
        + train_test_labels[
            lll] + '_' +
...
plt.draw()
plt.savefig(
    'C:\\Users\\Asus\\Desktop\\Project\\Comparing Models Last Layer\\BX_BX\\bodyface\\Brain BX_BX_Accuracy 0_900\\' + 'BX_BX_' +
    model_names + ' Accuracy.png',
    dpi=300)
plt.close()
# </editor-fold>
...
plt.errorbar(time, average, yerr=all_stds, ecolor='blue', color='red', linewidth=8,
             label='average')
plt.legend(fontsize=30)
plt.xticks(np.arange(0, len(time), 10), fontsize=30, rotation=90)
plt.ylabel("Accuracy", fontsize=30)
# plt.title("Train / Test", fontsize=55)
plt.legend(fontsize=25)
plt.draw()
plt.savefig(
    ...
    model_names + ' Accuracy.png',bbox_inches='tight',
    dpi=300)
plt.close()
# </editor-fold>
