...
for kkk in range(len(neuron_output)):
    ...
    train_input_BH = np.transpose(
        neuron_output[kkk]["neurons_stacked_averaged_for_" + str(0 + 5 * kkk) + "_to_" + str(49 + 5 * kkk)][:,
        np.r_[74:80, 83:101]])
    train_input_BI = np.transpose(
        neuron_output[kkk]["neurons_stacked_averaged_for_" + str(0 + 5 * kkk) + "_to_" + str(49 + 5 * kkk)][:,
        np.r_[101:107, 110:128]])
    train_input_BL = np.transpose(
        neuron_output[kkk]["neurons_stacked_averaged_for_" + str(0 + 5 * kkk) + "_to_" + str(49 + 5 * kkk)][:,
        np.r_[128:134, 137:155]])
...
plt.savefig(
    'C:\\Users\\Asus\\Desktop\\Project\\Comparing Models Last Layer\\BX_BX\\BFNM double human face\\'
    'Brain BX_BX_Accuracy 0_900\\' + 'BX_BX_' +
    model_names + ' Accuracy.png',
    dpi=300)
plt.close()
# </editor-fold>
