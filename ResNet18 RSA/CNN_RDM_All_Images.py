...
for k in range(len(slice_names)):
    sliceF_x_image = pd.read_pickle(
        r'C:\Users\Asus\Desktop\Project\ResNet18\CNN Slices Flattened by Image as pickle\\' + str(k).zfill(3) + '_' +
        slice_names[
            k] + '_for_all_images' + '.p')
    sliceF_x_image = sliceF_x_image.transpose()
    sliceF_x_image = sliceF_x_image[77:83]
    ...
    ax = sns.heatmap(matrix, xticklabels=x_axis_labels, yticklabels=y_axis_labels,
                     linewidth=0.5, cmap='afmhot')
    plt.draw()
    plt.savefig(
        'C:\\Users\\Asus\\Desktop\\Project\\ResNet18\\RDMs CNN all images plot 2\\' + str(k).zfill(
            3) + '_RDM_' +
...
#     sliceF_x_image = pd.read_pickle(
#         r'C:\Users\Asus\Desktop\Project\ResNet18\CNN Slices Flattened by Image as pickle\\' + str(w).zfill(3) + '_' +
#         slice_names[
#             w] + '_for_all_images' + '.p')
#     sliceF_x_image = sliceF_x_image.transpose()
#
#     sliceF_x_image = sliceF_x_image[77:89]
#     slice_data = rsatoolbox.data.Dataset(sliceF_x_image)
...
#     plt.draw()
#     plt.savefig(
#         'C:\\Users\\Asus\\Desktop\\Project\\ResNet18\\RDMs CNN all images plot 2\\' + str(w).zfill(3) + '_RDM_' +
