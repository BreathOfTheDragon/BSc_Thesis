...
for j in range(len(slice_names)):
    ...
    for i in range(len(image_names_notif)):
        image_dictionary = pd.read_pickle(
            r'C:\\Users\\Asus\\Desktop\\Project\\MobileNet-V2\\CNN Layer outputs in tensor as pickle\\' +
            image_names_notif
            [
                i] + '_tensor.p')
        image_by_slice.append(image_dictionary[slice_names[j]].flatten().cpu())
        print("image", k)
        k += 1
    ...
    pickle.dump(slice_by_image_stacked,
                open("C:\\Users\\Asus\\Desktop\\Project\\MobileNet-V2\\CNN Slices Flattened by Image as pickle\\" + str(
                    j) + '_' +
                     slice_names[
