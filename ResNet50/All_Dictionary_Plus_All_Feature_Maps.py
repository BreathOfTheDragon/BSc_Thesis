...
#     res50.eval()
#     res50(images_transformed_un_squeezed[i])
#     j = 0
#     for j in range(slice_names.size):
#         slice_output = create_feature_extractor(res50, return_nodes=[slice_names[j]])
#         with torch.no_grad():
#             slice_outputs[slice_names[j]] = slice_output(images_transformed_un_squeezed[i])[slice_names[j]].cpu()
#     all_images_all_layers[image_names_notif[i]] = slice_outputs
#     k += 1
#     print(k)
#     pickle.dump(all_images_all_layers[image_names_notif[i]],
#                 open("C:\\Users\\Asus\\Desktop\\Project\\ResNet50\\CNN Layer outputs in tensor as pickle\\" +
#                      image_names_notif[
#                          i] + '_tensor.p', "wb"))
...
for Q in range(len(images)):
    ...
    image_dictionary = pd.read_pickle(
        r'C:\\Users\\Asus\\Desktop\\Project\\ResNet50\\CNN Layer outputs in tensor as pickle\\' + image_names_notif[
            Q] + '_tensor.p')
    ...
    for i in range(len(processed)):
        a = fig.add_subplot(6, 9, i + 1)
        plt.imshow(processed[i])
        a.axis("off")
        a.set_title(conv_slices[i].split('(')[0], fontsize=20)
    plt.savefig('C:\\Users\\Asus\\Desktop\\Project\\ResNet50\\Images feature maps\\' + image_names_notif[Q] + '.png',
                bbox_inches='tight')
    plt.close()
