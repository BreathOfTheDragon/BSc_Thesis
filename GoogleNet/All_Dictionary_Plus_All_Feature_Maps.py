...
for i in range(len(images)):
    ...
    googlenet.eval()
    googlenet(images_transformed_un_squeezed[i])
    j = 0
    for j in range(slice_names.size):
        slice_output = create_feature_extractor(googlenet, return_nodes=[slice_names[j]])
        with torch.no_grad():
    ...
    pickle.dump(all_images_all_layers[image_names_notif[i]],
                open("C:\\Users\\Asus\\Desktop\\Project\\GoogleNet\\CNN Layer outputs in tensor as pickle\\" +
                     image_names_notif[
                         i] + '_tensor.p', "wb"))
...
for Q in range(len(images)):
    ...
    for i in range(len(processed)):
        a = fig.add_subplot(15, 15, i + 1)
        plt.imshow(processed[i])
        a.axis("off")
        a.set_title(conv_slices[i].split('(')[0], fontsize=7)
    plt.savefig('C:\\Users\\Asus\\Desktop\\Project\\GoogleNet\\Images feature maps\\' + image_names_notif[Q] + '.png',
                bbox_inches='tight')
    plt.close()
