...
#     plt.savefig('E:\\Users\\Asus\\Desktop\\Project\\ResNet18\\Images transformed shown in RGB\\'
...
#     res18.eval()
#     res18(images_transformed_un_squeezed[i])
#     j = 0
#     for j in range(slice_names.size):
#         slice_output = create_feature_extractor(res18, return_nodes=[slice_names[j]])
#         with torch.no_grad():
...
#                 open("E:\\Project\\ResNet18\\CNN Layer outputs in tensor as pickle\\" +
#                      image_names_notif[
#                          i] + '_tensor.p', "wb"))
...
for Q in range(len(images)):
    ...
    image_dictionary = pd.read_pickle(
        r'C:\\Users\\Asus\\Desktop\\Project\\ResNet18\\CNN Layer outputs in tensor as pickle\\' +
        image_names_notif[
            Q] + '_tensor.p')
    ...
    for i in range(len(processed)):
        a = fig.add_subplot(2, 9, i + 1)
        plt.imshow(processed[i])
        a.axis("off")
        a.set_title(conv_slices[i].split('(')[0], fontsize=25)
    plt.savefig('C:\\Users\\Asus\\Desktop\\Project\\ResNet18\\Images feature maps\\' + image_names_notif[Q] + '.png',
                bbox_inches='tight')
    plt.close()
